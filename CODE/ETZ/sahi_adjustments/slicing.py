...
def slice_image(
    ...
    exif_fix: bool = True,
    # ADJUSTMENT BY LOTTE MICHELS
    # THESE THREE FUNCTION ARGUMENTS WERE ADDED:
    ROI_informed: bool = False, # boolean argument to indicate if patches are created by a sliding window (false) or bone detector (true)
    bone_detection_model = None, # if ROI_informed = True, a bone detector model (pre-trained YOLOv5) should be passed as an additional argument
    return_bboxes = False, # whether the bbox coordinates should also be returned (true) or not (false). Used only in the SF augmentation script.
) -> SliceImageResult:
    ...
    ### ADJUSTMENT BY LOTTE MICHELS (start)
    # If-else statement depending on whether slicing should be done with a sliding window (default) or for bone tissue only (ROI-informed)
    if not ROI_informed: # if ROI_informed is false, apply the sliding window slicing as the original software does!
    ...
    elif ROI_informed: # if ROI_informed is true, apply the bone detector to retrieve bone bounding boxes and use those to create patches!
        slice_bboxes = []
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        bones = bone_detection_model(image, iou=0.5, conf=0.393, device=device) # detected bones
        for path in glob.glob("/tmp/pymp*"):
            subprocess.run(['rm', '-r', path], capture_output=True)
        for bone in bones:
            bones_tensor = bone.boxes.xyxy
            bone_boxes = bones_tensor.cpu().numpy()
            for i, bone_box in enumerate(bone_boxes): # convert bone boxes to the original slicing format
                bone_box = bone_box.astype(int)
                xmin, ymin, xmax, ymax = bone_box
                slice_bboxes.append([xmin, ymin, xmax, ymax])
    ### ADJUSTMENT BY LOTTE MICHELS (end)
    ...
    if return_bboxes:
        return sliced_image_result, slice_bboxes
    else:
