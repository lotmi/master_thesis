"""
Script to evaluate a YOLO Model with (ROI-)SAHI inference.
...
test_image_path = "/wecare/home/lotte/Thesis/DATA/ETZ/YOLO_NS_ROISF/images/test/" # test datapath
test_images = os.listdir(test_image_path)
test_target_path = "/wecare/home/lotte/Thesis/DATA/ETZ/PASCAL_Sample/test/annotations.json" # test labelpath
...
for test_image in tqdm(test_images):
    ...
    subject_lesion = subject_nr+'_'+take+'_'+lesion_nr # e.g. 89_1_l3
    if ROI_informed:
        sahi_results = get_sliced_prediction(test_image_path+test_image , detection_model, 
                                            ROI_informed=True, bone_detection_model=bone_detection_model,
                                            postprocess_match_threshold=iou, verbose=0)  
...
TP_dataframe = pd.DataFrame(TP_dictionary) # Save DR ratios per lesion to a separate file
if ROI_informed: 
