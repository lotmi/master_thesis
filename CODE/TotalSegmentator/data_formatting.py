...
def to_yolo_format(label_outputpath, propspath, datasplit, singular=False):
    ...
    props_df = pd.read_csv(propspath, dtype={'scan_nr': str})
    for row, props in props_df.iterrows():
        row, scan_nr, view, slice_index, index, label, area, centroid_0, centroid_1, bbox_0, bbox_1, bbox_2, bbox_3, im_width, im_height = props.to_list()
        for split, (low, high) in datasplit.items():
            if low <= int(scan_nr) <= high:
                scan_split = split
                break
        ...
        txt_filename = '{}_{}_{}.txt'.format(scan_nr, view, slice_index)
        ...
        if txt_filename in os.listdir(txt_path) and not singular: # If text file already exists (this slice contains multiple bones): append 
            ...
            txt_filename = '{}_{}_{}_n{}.txt'.format(scan_nr, view, slice_index, int(label))
...
def to_pascal_format(pascal_path, propspath, datasplit): 
    ...
    for row, props in props_df.iterrows():
        ...
        row, scan_nr, view, slice_index, index, label, area, centroid_0, centroid_1, y_min, x_min, y_max, x_max, im_width, im_height = props.to_list()
        img_filename = '{}_{}_{}.png'.format(scan_nr, view, slice_index)
        bbox = [x_min, y_min, x_max, y_max]
        
        for (split, (low, high)), data_dic in zip(datasplit.items(), data_dics):
            if low <= int(scan_nr) <= high:
